...
class NodeEmbedding(nn.Module):
    def __init__(self, node_size, node_dim=64, sequence_len=64, batch_size=128):
        """By entering a frequency query as the key of the query dictionary,
        the corresponding node vector representation in the dictionary is
        obtained.
...
class STGCNEncoder(nn.Module):
    def __init__(self, st_layers=2, input_node_channel=16, conv_node_channel=16, end_node_channel=32, output_node_channel=64, gcn_depth=2, hopalpha=0.05, dropout=0.2):
        """Dimensions of latent space--the number of layers of ST
        convolution."""
    ...
    def forward(self, x, adj, training=False):
        ...
        print(self.en_output_conv2(stc_output).shape)
...
class WaveDecompose(nn.Module):
    """Multilevel DWT return frequency components."""
...
class WaveGCN(nn.Module):
    """Input three different levels of Wave input, return three GCN outputs."""
...
class MultiLevelWaveGCN(nn.Module):
    """
    ...
    def __init__(self, input_channel=8, gcn_depth=1, hopalpha=0.05):
        super(MultiLevelWaveGCN, self).__init__()
