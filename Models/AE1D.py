...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        x = nn.functional.pad(x, (1, 1))  # Zero padding only on the right side for PyTorch
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #decoded = torch.narrow(decoded, -1, 1, 300)
        return decoded


class Autoencoder(nn.Module):
    def __init__(self, first_direction, sfreq, window_size, reg2):
